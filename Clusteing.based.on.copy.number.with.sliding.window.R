...
### append data from two and three adjacent bins
...
for (j in 2:3) {
	sliding <- apply(data_matrix_norm, 2, rollsum.median, i=j)
	remove.indexes <- c()
	for (k in 1:(j - 1)) {
		remove.indexes <- c(remove.indexes, chrom.starts - k)
	}
	cat(nrow(sliding), length(remove.indexes), length(unique(remove.indexes)), "\n")
	combined_matrix <- rbind(combined_matrix, sliding[-remove.indexes, ])
}
